...
<div class="eq-band">
  ...
  <div>
    <div class="eq-band-title">{sec.get('title', '🏭 石化设备寻源与采购价格分析')}</div>
...
""", unsafe_allow_html=True)
            if st.button(f"进入 {c['cn']} · 工厂选择 + 价格分析 →",
...
def _plant_map(c: dict):
    ...
    for p in _plants():
        ...
        runners = ranked[1:3]
        bk = ("<br>🔁 备选：" + "、".join(f"{b['name']}({b['score']:.0f})" for b in runners)) if runners else ""
        band = (f"{_fmt(pa['rec_low'])}–{_fmt(pa['rec_high'])} 万元（锚定 {_fmt(pa['anchor'])}）"
                if pa else "—")
        hover = (f"<b>SABIC {p['cn']} 工厂</b><br>{p.get('feature','')}<br>"
                 f"🥇 首选：<b>{prim.get('name','—')}</b> · <b>{prim.get('score',0):.1f} 分</b><br>"
                 f"💰 建议采购价：<b>{band}</b>"
                 f"<br><span style='color:#9fb3c8'>置信 {pa.get('confidence','—')}% · 多源交叉验证</span>{bk}")
...
def _convergence_fig(pa: dict):
    ...
    for s in pa.get("sources", []):
        rows.append((f"{s['short']}（{s['samples']}条）", s["low"], s["avg"], s["high"], "#2563eb"))
    sq = pa.get("supplier_quote", {})
    if sq:
        rows.append((f"供应商报价（{sq.get('n',0)}家）", sq["low"], sq["p50"], sq["high"], "#0891b2"))
    ...
    for i, (lbl, lo, avg, hi, col) in enumerate(rows):
        ...
        fig.add_trace(go.Scatter(
            ...
            hovertemplate=f"{lbl}<br>区间 {_fmt(lo)}–{_fmt(hi)} 万<br>均值 <b>{_fmt(avg)}</b> 万<extra></extra>",
            showlegend=False,
        ))
    fig.update_layout(
        font=_FONT, paper_bgcolor=_BG, plot_bgcolor="#fbfdff",
        height=58 * len(rows) + 90, margin=dict(l=10, r=20, t=42, b=10),
        xaxis=dict(title="到厂价（万元）", gridcolor="#eef2f7", zeroline=False),
        yaxis=dict(automargin=True),
        annotations=[
            dict(x=pa["anchor"], y=1.06, yref="paper", showarrow=False,
                 text=f"◆ 交叉锚定 {_fmt(pa['anchor'])} 万", font=dict(color="#0E8C3A", size=12.5)),
            dict(x=pa["ref_price"], y=-0.16, yref="paper", showarrow=False,
                 text=f"行业均价 {_fmt(pa['ref_price'])}", font=dict(color="#94a3b8", size=11)),
        ],
    )
    return fig
...
def _trend_fig(pa: dict):
    ...
    fig.add_trace(go.Scatter(
        ...
        hovertemplate="%{x} 年中标均价 <b>%{y} 万</b><extra></extra>",
        showlegend=False,
    ))
    fig.add_hline(y=pa["anchor"], line=dict(color="#0E8C3A", width=1.6, dash="dash"),
                  annotation_text=f"当期锚定 {_fmt(pa['anchor'])} 万",
    ...
    fig.update_layout(
        ...
        yaxis=dict(title="中标均价（万元）", gridcolor="#eef2f7"),
    )
    return fig
...
def _dim_bars_html(sup: dict) -> str:
    ...
    for k in DIM_KEYS:
        ...
        rows += (
            f"<div class='eq-dim'>"
            f"<span class='eq-dim-l'>{DIM_CN[k]}</span>"
...
def _render_plant_podium(c: dict, p: dict) -> None:
    ...
    for i, sup in enumerate(ranked[:3]):
        ...
        chips = (f"<span class='eq-pt' style='--bcol:{col}'>{sup['qualification']}</span>"
                 f"<span class='eq-pt' style='--bcol:{col}'>{'本地' if sup['is_local'] else '跨区'}</span>"
                 f"<span class='eq-pt' style='--bcol:{col}'>{sup['lead_time_days']}天交付</span>")
        with cols[i]:
            st.markdown(
                ...
                f"<div class='eq-pod-est'>预估到厂价 <b>{_fmt(sup['est_price_wan'])} 万</b>"
                f" · 价格系数 {sup['price_level']:.2f} · 距厂 {sup['distance_km']} km</div>"
                f"<div class='eq-pod-note'>{sup['special_notes']}</div>"
                f"<div class='eq-pt-row'>{chips}</div>"
                f"</div>",
                unsafe_allow_html=True,
            )
    with st.expander(f"📋 展开 {p['cn']} 全部 {len(ranked)} 家 · 5 维评分明细"):
        for sup in ranked:
            sccol = _score_color(sup["score"])
            st.markdown(
                ...
                f"<span class='eq-detail-score' style='color:{sccol}'>{sup['score']:.1f} 分</span>"
                f"</div>"
...
def _render_price_report(c: dict, p: dict) -> None:
    ...
    delta_txt = (f"较行业均价 {'+' if delta >= 0 else '−'}{_fmt(abs(delta))} 万"
    ...
    st.markdown(
        ...
        f"<div class='eq-ph-l'>"
        f"<div class='eq-ph-kicker'>💰 {p['cn']} · {c['cn']} · 建议采购价区间（多源交叉验证）</div>"
        f"<div class='eq-ph-band'>{_fmt(pa['rec_low'])} <span>–</span> {_fmt(pa['rec_high'])} "
        f"<span class='eq-ph-unit'>万元 / 台</span></div>"
        f"<div class='eq-ph-anchor'>◆ 交叉锚定价 <b>{_fmt(pa['anchor'])} 万</b> · {delta_txt}</div>"
        f"</div>"
        f"<div class='eq-ph-r'>"
        f"<div class='eq-ph-conf' style='color:{conf_col}'>{conf}<span>%</span></div>"
        f"<div class='eq-ph-conf-lbl'>验证置信度</div>"
        f"<div class='eq-ph-samp'>{pa['total_samples']} 条多源样本 · 离散度 {pa['dispersion']}%</div>"
        f"</div>"
        f"</div>",
        unsafe_allow_html=True,
    )
    ...
    for i, s in enumerate(pa["sources"]):
        with scols[i]:
            st.markdown(
                ...
                f"<div class='eq-src-meta'>区间 {_fmt(s['low'])}–{_fmt(s['high'])} 万 · "
                f"<b>{s['samples']}</b> 条中标</div>"
                f"<div class='eq-src-period'>📅 {s['period']} 框架协议/中标公告</div>"
                f"</div>",
                unsafe_allow_html=True,
            )

    # ── 历史走势 ────────────────────────────────────────────────────
    st.markdown("##### 📈 2021–2024 中标价走势 · 回归至当期")
    tf = _trend_fig(pa)
    if tf is not None:
...
def render_equipment_report(cat_key: str) -> None:
    ...
    if st.button("← 返回设备寻源 / 全部品类", key="eq_back"):
...
<div class="eq-hero" style="--accent:{c['accent']}">
  <div class="eq-hero-l">
    ...
    <div>
      <div class="eq-hero-kicker">🏭 石化设备寻源 · 工厂选择 + 多源价格分析</div>
      <div class="eq-hero-title">{c['cn']}</div>
      <div class="eq-hero-tagline">{c['en']} · 📐 {c['spec']}</div>
    </div>
  </div>
  <div class="eq-hero-model">🧭 行业均价基准 {_fmt(c['ref_price_wan'])} 万元/台 · 价格来源：{c['source']}</div>
</div>
""", unsafe_allow_html=True)

    # ── 四大工厂总览地图 ────────────────────────────────────────────
    st.markdown("#### 🗺️ 四大工厂寻源总览 · 各厂首选供应商与建议采购价，一图直达")
    _map = _plant_map(c)
    if _map is not None:
        st.plotly_chart(_map, width="stretch",
                        config={"displayModeBar": False, "scrollZoom": True},
                        key=f"eq_map_{c['key']}")
        st.caption("◆ 菱形为四大交付工厂：上海浦东(绿) · 广州南沙(蓝) · 福建漳州古雷(橙) · 重庆(紫) —— "
                   "标注显示该厂交叉锚定建议价；悬停看『首选供应商 + 建议价区间 + 备选』。")

    # ── 工厂选择器 ──────────────────────────────────────────────────
    st.markdown("#### 🏭 选择交付工厂 · 查看入围供应商与采购价格分析报告")
    plants = _plants()
    ...
    st.markdown(
        ...
        f"<span class='eq-plant-name'>SABIC {p['cn']} 工厂</span>"
    ...
    # ── 入围供应商领奖台 ────────────────────────────────────────────
    st.markdown("##### 🏗️ 入围供应商排名 · Top3 推荐（5 维加权专家评分），可展开全部明细")
    ...
    # ── 评分框架 ────────────────────────────────────────────────────
    with st.expander("📐 供应商评分框架：5 维加权专家模型（价格/交期/属地/资质/规模）"):
        st.markdown(
...
.eq-dim-l{width:104px;font-size:11.5px;color:#475569;}
